...
#define ASSERT_ASSIGN(src, dst) \
    ASSERT(src) \
    (dst) = src;
#define ASSERT_LOG_ASSIGN(src, dst) \
    ASSERT_LOG(src, #dst) \
    (dst) = src;
...
namespace UnrealVR {
    ...
    void __cdecl UE4Service::CalculateProjectionMatrixGivenViewDetour(
        ...
        const auto m = static_cast<UE4::FMatrix*>(InOutProjectionData) + 0x50;
        *m = {
    ...
    void UE4Service::SetViewTarget() {
        ...
        ASSERT_LOG_ASSIGN(getViewTargetParams.ViewTarget, currentViewTarget)
        ...
        if (childViewTarget == nullptr) {
            ...
            ASSERT_LOG_ASSIGN(getComponentByClassParams.Result, cameraComponent)
        }
    ...
    void UE4Service::Resize(const int width, const int height) {
        ...
        ASSERT_LOG_ASSIGN(getGameUserSettingsParams.Result, gameUserSettings)
