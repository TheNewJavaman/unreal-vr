...
namespace UnrealVR
{
	namespace VRManager
	{
		...
		bool GetInstanceWithExtensions()
		{
			...
			Log::Info("OpenXR extensions available:");
			for (size_t i = 0; i < allExtensions.size(); i++)
			{
				Log::Info(std::format("- {}", allExtensions[i].extensionName));
			...
			{
				Log::Error("The active OpenXR runtime does not support D3D11");
				return false;
			}
			...
			{
				Log::Error("Failed to create OpenXR instance");
				return false;
			}
			return true;
		}
		...
		void LoadDebugExtension()
		{
			...
			debugInfo.userCallback = [](XrDebugUtilsMessageSeverityFlagsEXT severity, XrDebugUtilsMessageTypeFlagsEXT types, const XrDebugUtilsMessengerCallbackDataEXT* msg, void* user_data) {
				Log::Info(std::format("OpenXR | {}: {}", msg->functionName, msg->message));
			...
			Log::Info("Loaded OpenXR debug extension");
		}
		...
		void LoadD3D11Extension()
		{
			...
			Log::Info("Partially loaded OpenXR D3D11 extension");
		}

		bool ContinueInitAndCreateSwapChain()
		{

			Log::Info("Finished loading OpenXR D3D11 extension");
			return true;
		}
		...
		void Stop()
		{
			...
			{
				Log::Error("Failed to destroy OpenXR instance");
			}
			if (xrDebug != XR_NULL_HANDLE && xrExtDestroyDebugUtilsMessenger(xrDebug) != XR_SUCCESS)
			{
				Log::Error("Failed to destroy OpenXR debug");
			}
		}
	}
}